<%-- src/main/webapp/WEB-INF/views/message/important_inbox.jsp --%>
...
<head>
    ...
    <style>
        ...
        .favorite-icon.active { color: gold; }
...
<c:set var="currentPage" value="important" scope="request" />
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        async function fetchAndRenderMessages(page = 1, size = 10) {
            ...
            try {
                // MessageRestController의 /api/v1/messages/important API 호출
                const response = await fetch(`/api/v1/messages/important?page=\${page}&size=\${size}`);
                if (!response.ok) {
                ...
                console.log('중요 쪽지 목록 데이터:', paginationResponse);

                renderMessageTable(paginationResponse.content);
                renderPagination(paginationResponse);

            } catch (error) {
                console.error('중요 쪽지 목록을 불러오는 중 오류 발생:', error);
        ...
        function renderMessageTable(messages) {
            if (!messages || messages.length === 0) {
                messageListArea.innerHTML = '<p class="text-center text-muted">중요 표시된 쪽지가 없습니다.</p>';
                return;
            }
            ...
            messages.forEach(msg => {
                ...
                // 중요 쪽지함이므로 항상 active 아이콘 (중요함 해제 기능은 받은 쪽지함에서만 직접적으로 나타남)
                const importantIconHtml = '<span class="favorite-icon active">★</span>';
        ...
        function attachTableEventListeners() {
            // 중요 표시/해제 버튼 (중요 쪽지함에서는 주로 해제만 가능)
            document.querySelectorAll('.favorite-form button').forEach(button => {
                button.addEventListener('click', async function() {
                    ...
                    // 중요 쪽지함에서는 클릭 시 항상 'N'으로 변경 요청 (중요 해제)
                    const newImportantYn = 'N';
