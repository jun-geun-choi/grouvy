...
.file-list-actions .restore-btn {
    background: #28a745;
    color: #fff;
    border: none;
}

.file-list-actions .restore-btn:hover {
    background: #218838;
}
...
<%--    <div class="file-search-group">--%>
<%--    <span class="date-label">삭제일</span>--%>
...
<%--</div>--%>
                ...
                <div class="file-list-box">
                    <div class="file-list-header">
                        <span class="file-list-title">휴지통 목록</span>
                        <div class="file-list-actions">
                            <button type="submit" id="restore-btn" class="restore-btn" formaction="/file/trash/restore">복원</button>
                            <button type="submit" id="delete-btn" class="delete-btn" formaction="/file/trash/delete">영구삭제</button>
                        </div>
                    </div>
                    <table class="file-table">
                        ...
                        <c:forEach var="trash" items="${trashList}">
                            <tr data-file-id="1">
                                ...
                                <td>
                                    <c:if test="${trash.file.ownerType == 'personal'}">개인</c:if>
                                    <c:if test="${trash.file.ownerType == 'department'}">부서</c:if>
                                </td>
                                <td>${trash.file.fileCategoryName}</td>
                                <td>${trash.file.originalName}</td>
                                <td>
                                    <c:set var="size" value="${trash.file.size}" />
...
<%@ include file="../common/footer.jsp" %>
    ...
    document.getElementById('button-form').addEventListener('submit', function(e) {
        const checked = document.querySelectorAll('input[name="trashIds"]:checked');
        if (checked.length === 0) {
    ...
    document.getElementById('restore-btn').addEventListener('submit', function(e) {
        const checked = document.querySelectorAll('input[name="trashIds"]:checked');
        if (checked.length === 0) {
            alert('복원 할 파일을 선택하세요.');
            e.preventDefault();
        }
    });
    
    document.getElementById('delete-btn').addEventListener('submit', function(e) {
        const checked = document.querySelectorAll('input[name="trashIds"]:checked');
        if (checked.length === 0) {
            alert('삭제할 파일을 선택하세요.');
            e.preventDefault();
